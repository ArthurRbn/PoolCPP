...
Droid::Droid(std::string serial, size_t Energy, size_t Attack, size_t Toughness, std::string status): _Id(serial),
                                                                                                      _energy(Energy),
                                                                                                      _attack(Attack),
...
{
    if (_Id == toCompare.getId() && _energy == toCompare.getEnergy() && _attack == toCompare.getAttack()
        && _toughness == toCompare.getToughness() && *_status == *toCompare.getStatus())
        return true;
    else
        return false;
}
